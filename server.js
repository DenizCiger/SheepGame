...
setInterval(() => {
    ...
    // Realistic sheep collision (using mass and elasticity)
    ...
    for (let i = 0; i < ids.length; i++) {
        ...
        for (let j = i + 1; j < ids.length; j++) {
            ...
            const minDist = SHEEP_RADIUS * 2;
            if (distSq < minDist * minDist && distSq > 0.001) {
                const dist = Math.sqrt(distSq);
                const nx = dx / dist; // Normal from B to A
                const nz = dz / dist;

                // Separation step to prevent phasing
                const overlap = minDist - dist;
                const separationX = nx * overlap * 0.5; // Separate by half the overlap each
                ...
                // Collision response (velocity and angular velocity)
                const dvx = a.velocityX - b.velocityX;
                const dvz = a.velocityZ - b.velocityZ;
                const vRelNorm = dvx * nx + dvz * nz; // Relative velocity along the normal

                if (vRelNorm < 0) { // Objects are moving towards each other
                    // Linear Impulse (same as before)
                    const impulseMag = -(1 + ELASTICITY) * vRelNorm * MASS / 2;
                    const impulseX = nx * impulseMag;
                    const impulseZ = nz * impulseMag;

                    // Apply linear impulse
                    a.velocityX += impulseX / MASS;
                    a.velocityZ += impulseZ / MASS;
                    b.velocityX -= impulseX / MASS;
                    b.velocityZ -= impulseZ / MASS;

                    // --- Rotational Impulse (Torque causing tilt, with region sensitivity) ---
                    // Axis of rotation is perpendicular to normal (nx, 0, nz) and up (0, 1, 0) -> (nz, 0, -nx)
                    // Torque magnitude is related to impulse magnitude, lever arm (radius), and multiplier

                    // Calculate forward vector for each sheep
                    const aForwardX = Math.sin(a.rotation);
                    const aForwardZ = Math.cos(a.rotation);
                    const bForwardX = Math.sin(b.rotation);
                    const bForwardZ = Math.cos(b.rotation);

                    // Dot product of normal and forward vector tells us if hit is at head (1), side (0), or rear (-1)
                    const aDot = nx * aForwardX + nz * aForwardZ;
                    const bDot = -nx * bForwardX + -nz * bForwardZ; // Reverse normal for b

                    // Interpolate sensitivity: front = COLLISION_SENSITIVITY_FRONT, side = COLLISION_SENSITIVITY_SIDE
                    // function lerp(a, b, t) { return a + (b - a) * t; }
                    const aSensitivity = lerp(PHYSICS.COLLISION_SENSITIVITY_SIDE, PHYSICS.COLLISION_SENSITIVITY_FRONT, Math.max(0, aDot));
                    const bSensitivity = lerp(PHYSICS.COLLISION_SENSITIVITY_SIDE, PHYSICS.COLLISION_SENSITIVITY_FRONT, Math.max(0, bDot));

                    const torqueMagA = Math.abs(impulseMag) * SHEEP_RADIUS * COLLISION_TILT_MULTIPLIER * aSensitivity;
                    const torqueMagB = Math.abs(impulseMag) * SHEEP_RADIUS * COLLISION_TILT_MULTIPLIER * bSensitivity;

                    // Change in angular velocity = Torque / MomentOfInertia (approximated using MASS)
                    const deltaAVX_A = torqueMagA * nz / MASS; // Torque around X axis depends on Z normal
                    const deltaAVZ_A = torqueMagA * -nx / MASS; // Torque around Z axis depends on -X normal

                    // Apply angular velocity change to A
                    a.angularVelocityX = (a.angularVelocityX || 0) + deltaAVX_A;
                    a.angularVelocityZ = (a.angularVelocityZ || 0) + deltaAVZ_A;

                    // Apply opposite torque to sheep B (with its own sensitivity)
                    const deltaAVX_B = torqueMagB * -nz / MASS;
                    const deltaAVZ_B = torqueMagB * nx / MASS;
                    b.angularVelocityX = (b.angularVelocityX || 0) + deltaAVX_B; // Line 437 (approx)
                    b.angularVelocityZ = (b.angularVelocityZ || 0) + deltaAVZ_B;

                    // --- START: Trigger Timid Scare ---
                    const SCARE_DURATION = 2.0; // How long the sheep stays scared (in seconds)
                    const SCARE_JUMP_STRENGTH = 0.6; // Increased jump strength (adjust as needed)

                    if (a.personality === 'timid' && a.scaredTimer <= 0) {
                        a.scaredTimer = SCARE_DURATION;
                        // Remove setting scaredAngle
                        // a.scaredAngle = Math.atan2(nx, nz) + (Math.random() - 0.5) * 0.5;
                        // Add vertical jump impulse
                        a.velocityY += SCARE_JUMP_STRENGTH;
                    }
                    if (b.personality === 'timid' && b.scaredTimer <= 0) {
                        b.scaredTimer = SCARE_DURATION;
                        // Remove setting scaredAngle
                        // b.scaredAngle = Math.atan2(-nx, -nz) + (Math.random() - 0.5) * 0.5;
                        // Add vertical jump impulse
                        b.velocityY += SCARE_JUMP_STRENGTH;
                    }
                    // --- END: Trigger Timid Scare ---
                }
            }
        }
    }
